...
import { updateUserMe } from "@/api-service/me.service";
...
const MeModal = ({  open, setOpen,modalOpen }: any) => {
        ...
        <Box sx={style}>
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            ...
            <div className="flex flex-col items-center relative">
                <img
                  src={`http://localhost:8080/${modalOpen?.avatar}` ? `http://localhost:8080/${modalOpen?.avatar}` : "/image.png"}
                  alt="image"
              ...
              <div className="flex flex-col mt-[50px]">
                <form action={handleModal} className="flex flex-col gap-[10px] items-center">
                  ...
                  <Button variant="contained" className='p-[10px] bg-[#151c2c] text-white rounded-sm w-[63%]'>Update</Button>
                </form>
              </div>
            </div>
          </Typography>
